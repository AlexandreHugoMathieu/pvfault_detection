...
def detect_shading_dynamic(power_timeseries, latitude, longitude, n_clusters=2):
    """
    Detect shading effects on a PV power time series based on dynamic clustering of solar position.

    Parameters:
    - power_timeseries: Pandas DataFrame with a datetime index and a 'power' column representing the PV power output.
    - latitude: Latitude of the PV system location.
    - longitude: Longitude of the PV system location.
    - n_clusters: Number of clusters for K-means clustering (default is 2).

    Returns:
    - Pandas DataFrame with an additional 'shading' column indicating shading periods (True) and non-shading periods (False).
    """

    # Calculate solar position
    times = power_timeseries.index
    solar_position = pvlib.solarposition.get_solarposition(times, latitude, longitude)

    # Extract elevation and azimuth angles
    solar_elevation = solar_position['elevation'].values
    solar_azimuth = solar_position['azimuth'].values

    # Combine elevation and azimuth into a feature matrix
    feature_matrix = np.column_stack((solar_elevation, solar_azimuth))

    # Apply K-means clustering
    kmeans = KMeans(n_clusters=n_clusters, random_state=0)
    clusters = kmeans.fit_predict(feature_matrix)

    # Determine the cluster with the lower solar elevation (potentially representing shading)
    shading_cluster = np.argmin(np.mean(feature_matrix[clusters == 0, 0]))

    # Identify shading periods based on the selected cluster
    shading_periods = (clusters == shading_cluster)

    # Create a DataFrame with shading information
    shading_info = pd.DataFrame(index=times, data={'shading': shading_periods})

    # Merge shading information with the original power time series
    power_timeseries_with_shading = pd.concat([power_timeseries, shading_info], axis=1)

    return power_timeseries_with_shading

# Example usage:
# Specify your PV system location (latitude, longitude)
latitude = 37.7749  # Example latitude (San Francisco, CA)
longitude = -122.4194  # Example longitude (San Francisco, CA)

# Generate a sample PV power time series (replace this with your actual data)
date_rng = pd.date_range(start='2023-01-01', end='2023-12-31', freq='H')
power_data = 100 * (1 + 0.2 * (pd.Series(range(len(date_rng))) - len(date_rng) / 2) / len(date_rng))
power_data.index = date_rng

# Detect shading effects dynamically
result = detect_shading_dynamic(pv_power_timeseries, latitude, longitude)
